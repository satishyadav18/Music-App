...
// Array of songs and their titles
const songs = [
...
let songIndex = 0;
let lastVolume = 0.5; // Default volume when not muted
...
volumeIcon.addEventListener('click', () => {
    if (audio.volume > 0) {
        ...
        audio.volume = 0;           
        volumeBar.value = 0;        
        volumeBar.style.width = '30px'; // Set smaller width when muted
    } else {
        audio.volume = lastVolume;  
        volumeBar.value = lastVolume * 100;  
        volumeBar.style.width = '100%'; // Set width to 100% when unmuted
    }
    updateVolume();  
});
...
// Load first song on page load and set initial volume
window.addEventListener('load', () => {
    loadSong(songs[songIndex]);
    volumeBar.value = 50;
    audio.volume = 0.5;  
    volumeBar.style.width = '100%'; // Set initial width to 100%
    updateVolume();  
});
